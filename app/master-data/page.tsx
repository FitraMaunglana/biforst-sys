    ...
    const handleSaveEdit = async () => {
        ...
        try {
            ...
            const { error: errLokasi } = await supabase
                .from('titik_lokasi')
                .update({ dukcapil_name: editForm.dukcapil_name.trim(), isp_name: editForm.isp_name })
                .eq('id', editingId);

            if (errLokasi) {
                // Constraint unik (kabupaten_id, dukcapil_name) akan menolak kalau
                // nama itu sudah dipakai titik lain di kabupaten yang sama.
                if (errLokasi.code === '23505') {
                    throw new Error(`Nama "${editForm.dukcapil_name}" sudah dipakai titik lain di kabupaten ini.`);
                }
                throw errLokasi;
            }
            ...
            <div className="flex-1 min-w-0 p-6">
                <div className="max-w-7xl mx-auto space-y-6">
                    ...
                    <div className="flex bg-slate-200/60 p-1.5 rounded-xl max-w-md border border-slate-200">
                        <button
                            onClick={() => setActiveMasterTab('harga')}
                            className={`flex-1 py-2 text-xs font-bold rounded-lg transition ${activeMasterTab === 'harga' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-600 hover:text-slate-900'}`}
                        >
                            Harga & ISP
                        </button>
                        <button
                            onClick={() => setActiveMasterTab('akun')}
                            className={`flex-1 py-2 text-xs font-bold rounded-lg transition ${activeMasterTab === 'akun' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-600 hover:text-slate-900'}`}
                        >
                            Kategori Akun
                        </button>
                        <button
                            onClick={() => setActiveMasterTab('proyek')}
                            className={`flex-1 py-2 text-xs font-bold rounded-lg transition ${activeMasterTab === 'proyek' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-600 hover:text-slate-900'}`}
                        >
                            Proyek & Kabupaten
                        </button>
                    </div>
                                                    ...
                                                    <tr key={titik.id} className={isEditing ? "bg-indigo-50 ring-2 ring-indigo-300 ring-inset" : "hover:bg-slate-50 transition"}>
                                                        <td className="px-4 py-4">
                                                            {isEditing ? (
                                                                <input
                                                                    type="text"
                                                                    value={editForm.dukcapil_name}
                                                                    onChange={e => setEditForm({ ...editForm, dukcapil_name: e.target.value })}
                                                                    className="w-32 p-1.5 border border-indigo-300 rounded text-xs font-bold"
                                                                />
                                                            ) : (
