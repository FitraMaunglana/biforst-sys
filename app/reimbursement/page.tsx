...
export default function ReimbursementPage() {
    ...
    const pendingCount = reimbursements.filter(r => r.status === 'Menunggu Review').length;
    const approvedTotal = reimbursements.filter(r => r.status === 'Disetujui').reduce((acc, curr) => acc + curr.amount, 0);

    const filteredReimbursements = reimbursements.filter(r => {
        if (activeTab === 'pending') return r.status === 'Menunggu Review';
        return true;
    });
                                            ...
                                            <tr key={r.id} className="hover:bg-rose-50/50 transition-colors group">
                                                ...
                                                <td className="px-6 py-4 text-center">
                                                    <div className="flex flex-col items-center gap-1.5">
                                                        ...
                                                        {r.status === 'Ditolak' && r.reject_reason && (
